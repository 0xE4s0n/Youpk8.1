      ...
      for (auto& m : methods) {
        ...
        if (!method->IsProxyMethod() && method->IsInvokable()) {
          ...
          if (!method->IsStatic()) {
            //patch by 0xE4s0n
            //+++++++++++++++++++++++++++
            ObjPtr<mirror::Object> thiz = klass->AllocObject(self);
...
void Unpacker::register_cn_e4s0n_Unpacker(JNIEnv* env) {
  REGISTER_NATIVE_METHODS("cn/e4s0n/Unpacker");
}
